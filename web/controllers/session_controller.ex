defmodule Peepchat.SessionController do
  ...
  def create(conn, %{"grant_type" => "password",
      ...
      ## Handle a user login
      try do
        # Try to get user from DB with matching email
        # to request (only one since uniqueness constraint)
        user = User
        ...
        cond do
          ...
          true ->
            ...
            conn
            |> put_status(401)
            |> render(Peepchat.ErrorView, "401.json") # 401 Not Authorized
        end
      rescue
        e ->
          IO.inspect e # Print to console...
